...
function normalizeProvider(value) {
  ...
  return UPDATE_PROVIDERS.has(provider) ? provider : 'xupdater';
}
...
class ProfileStore {
  ...
  #toPublicProfile(storedProfile) {
    ...
    const password = rememberAuth ? this.#decodeSecret(storedProfile.password) : '';
    ...
    const passwordUnavailable = rememberAuth && this.#isEncryptedSecret(storedProfile.password) && !password;

    return {
      ...storedProfile,
      provider: normalizeProvider(storedProfile.provider),
      channel,
      ignoreList,
      login,
      licenseKey,
      ...
      passwordUnavailable,
      inibuildsProductId: Number.isFinite(Number(storedProfile.inibuildsProductId))
        ? Math.trunc(Number(storedProfile.inibuildsProductId))
        : 0,
      inibuildsProductName: String(storedProfile.inibuildsProductName || '').trim().slice(0, 160),
      inibuildsActivationKey: String(storedProfile.inibuildsActivationKey || '').trim().slice(0, 200),
      credentialsUnavailable: loginUnavailable || licenseKeyUnavailable || passwordUnavailable
    };
  }
  ...
  #normalizeAndEncodeProfile(inputProfile, now) {
    ...
    const normalized = {
      ...
      password: rememberAuth ? this.#encodeSecret(inputProfile.password) : '',
      inibuildsProductId: Number.isFinite(Number(inputProfile.inibuildsProductId))
        ? Math.trunc(Number(inputProfile.inibuildsProductId))
        : 0,
      inibuildsProductName: String(inputProfile.inibuildsProductName || '').trim().slice(0, 160),
      inibuildsActivationKey: String(inputProfile.inibuildsActivationKey || '').trim().slice(0, 200),
      packageVersion: Number.isFinite(Number(inputProfile.packageVersion))
        ? Number(inputProfile.packageVersion)
        : 0,
      rememberAuth,
      provider: normalizeProvider(inputProfile.provider),
  ...
  async saveProfile(inputProfile) {
    ...
    const rememberAuth = Boolean(inputProfile.rememberAuth);
    const provider = normalizeProvider(inputProfile.provider);
    const hasLogin = Boolean(String(inputProfile.login || '').trim());
    const hasLicenseKey = Boolean(String(inputProfile.licenseKey || '').trim());
    const hasPassword = Boolean(String(inputProfile.password || '').trim());

    if (rememberAuth && !hasLogin) {
      throw new Error('Profile requires login when credentials are stored.');
    }

    if (rememberAuth && provider === 'xupdater' && !hasLicenseKey) {
      throw new Error('Profile requires licenseKey for X-Updater when credentials are stored.');
    }

    if (rememberAuth && provider === 'inibuilds' && !hasPassword && !hasLicenseKey) {
      throw new Error('Profile requires password (or fallback licenseKey) for iniBuilds when credentials are stored.');
    }
