...
class IniBuildsClient {
  ...
  async #downloadToFile(url, targetPath, signal, extraHeaders = null, progress = null, expectedTotalBytes = 0, pauseCheck = null) {
    ...
    try {
      ...
      const _isPaused = typeof pauseCheck === 'function' ? pauseCheck : null;
      const _isAborted = () => Boolean(signal && signal.aborted) || controller.signal.aborted;
      ...
      const counter = new Transform({
        transform: (chunk, _enc, cb) => {
          ...
          if (_isPaused && _isPaused()) {
            const waitLoop = () => {
              if (_isAborted()) {
                cb(this.#createInstallAbortError());
                return;
              }
              if (_isPaused()) {
  ...
  async createUpdatePlan(profile, options = {}) {
    ...
    const plan = {
      ...
      summary: {
        ...
        snapshotNumber: 0,
        fileCount: updateCount + deleteCount,
        deleteCount,
        downloadSize: knownDownloadSize,
        ...
        if (zipPath === tempDownload) {
          ...
          for (const headers of downloadHeaderCandidates) {
            try {
              ...
              if (dlError && dlError.code === 'INSTALL_CANCELLED') {
                throw dlError;
              }
              lastDownloadError = String(dlError && dlError.message ? dlError.message : dlError || 'unknown error');
            }
