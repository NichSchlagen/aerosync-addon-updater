...
function pickSnapshot(product, options, warnings) {
  ...
  if (options.alpha) {
    ...
    pushWarn(`Alpha angefordert, aber nicht verfuegbar fuer ${product.name}. Fallback aktiv.`);
    if (byType.has('beta')) {
  ...
  if (byType.has('release')) {
    if (options.beta && !byType.has('beta')) {
      pushWarn(`Beta angefordert, aber nicht verfuegbar fuer ${product.name}. Nutze Release.`);
    }
    return byType.get('release');
  }

  if (options.beta && !byType.has('beta')) {
    pushWarn(`Beta angefordert, aber nicht verfuegbar fuer ${product.name}. Nutze ersten Snapshot.`);
  }

  pushWarn(`Release fehlt fuer ${product.name}. Nutze ersten verfuegbaren Snapshot.`);
  return list[0];
}
...
function resolveProductDir(rootDir, location) {
  ...
  const base = path.resolve(root, location.path || '');
  const detection = Array.isArray(location.detection)
    ? location.detection.filter((item) => item && item.trim())
    : [];
  ...
  const direct = path.resolve(base, ...detection);
  ...
  while (cursor.startsWith(root)) {
    const candidate = path.resolve(cursor, ...detection);
...
class UpdateClient {
  ...
  async #buildPackageActions(productPackage, fresh) {
    const actions = [];
  ...
  async createUpdatePlan(inputProfile, options) {
    ...
    for (const product of allProducts) {
      ...
      const filesSince = pickedOptions.fresh ? 0 : profile.packageVersion;
      ...
      const productDir = resolveProductDir(profile.productDir, product.location);
    ...
    for (const productPackage of packages) {
      const part = await this.#buildPackageActions(productPackage, pickedOptions.fresh);
      actions.push(...part);
    }

    const sortedActions = [
      ...actions.filter((action) => action.type === 'delete'),
      ...actions.filter((action) => action.type === 'update')
    ];
