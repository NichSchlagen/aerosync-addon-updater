...
function resolveProductDir(rootDir, location) {
  ...
  const directCandidate = path.resolve(base, ...detection);
  const directAnchor = resolveDetectionAnchor(directCandidate);
  if (directAnchor) {
    return directAnchor;
  }
  ...
  while (cursor === root || cursor.startsWith(rootWithSep)) {
    ...
    const anchor = resolveDetectionAnchor(candidate);
    if (anchor) {
      return anchor;
    }
